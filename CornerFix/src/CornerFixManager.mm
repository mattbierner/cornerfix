    ...
    err:(void(^)()) err
{
    ...
    dng_win_glue* g = [self configureGlue: new dng_win_glue()];
    g->setBuildCPF();
    g->setImage(file);
        
    int result = g->dng_validate();
    if (result == dng_error_none)
        g->saveCPFFile([CornerFixFile convertExtension:file extension:@"cpf"]);
