...
void dng_info::ParseTag (dng_host &host,
	...
	if (makernotes && makernotes->ParseTag (stream,
						 	  parentCode,
						 	  tagCode,
						 	  tagType,
						 	  tagCount,
...
void dng_info::ParseIFD (dng_host &host,
	...
	if (gVerbose)
		...
		dng_local_printf::printf ("%s: Offset = %u, Entries = %u\n\n",
	...
	if (gVerbose)
		{
		dng_local_printf::printf ("NextIFD = %u\n", (unsigned) nextIFD);
		}
...
bool dng_info::ParseMakerNoteIFD (dng_host &host,
	...
	for (tagIndex = 0; tagIndex < ifdEntries; tagIndex++)
		...
		if (parentCode == tcLeicaMakerNote) {
			// On the M9, Leica have used subIFDs within the MakerNote
			// Horrible idea, but there you go
			switch ( tagCode) {
				case 0x3000:
				case 0x3100:
				case 0x3400:
				case 0x3900:
#if defined _DEBUG
				dng_local_printf::printf ("IFD in MakerNotes: 0x%x\n", tagCode);
#endif
				// We set parent code Leica in order to use this recursively
				if (ParseMakerNoteIFD (host,
										stream,
										tagCount,
										tagOffset,
										...
										tcLeicaMakerNote)) {
#if defined _DEBUG
					dng_local_printf::printf ("MakerNote subIFD processed ok\n");
#endif
					continue;
				}	
				break;
...
void dng_info::ParseMakerNote (dng_host &host,
	...
	if (memcmp (firstBytes, "LEICA\000\000\000", 8) == 0)
		...
		// The new Leica M9 MakerNote format.
		// No doubt someone had a reason for changing.....
		
		if (memcmp (firstBytes, "LEICA0\003\000", 8) == 0)
		{
...
void dng_info::ParseDNGPrivateData (dng_host &host,
	...
	while (section_offset + 8 < fShared->fDNGPrivateDataCount)
		...
		if (section_key == DNG_CHAR4 ('M','a','k','N') && section_count > 6)
			...
			if (gVerbose)
				{
				dng_local_printf::printf ("Found MakerNote inside DNGPrivateData\n\n");
				}
			...
			if (gVerbose)
				{
				dng_local_printf::printf ("Found Sony private data inside DNGPrivateData\n\n");
				}
			...
			if (gVerbose)
				{
				dng_local_printf::printf ("Found Fuji RAF tags inside DNGPrivateData\n\n");
				}
			...
			if (gVerbose)
				{
				dng_local_printf::printf ("Found Contax Raw header inside DNGPrivateData\n\n");
				}
			...
			if (parentCode)
				...
				if (gVerbose)
					{
					dng_local_printf::printf ("Found %s tags inside DNGPrivateData\n\n",
							LookupParentCode (parentCode));
					}
...
void dng_info::Parse (dng_host &host,
	...
	if (gVerbose)
		{
		dng_local_printf::printf ("Magic number = %u\n\n", (unsigned) fMagic);
		}
	...
	fMakerNotes.Reset (host.Make_dng_makernotes ());
