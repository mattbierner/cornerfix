...
void DumpHexAscii (dng_stream &stream,
	...
	for (uint32 row = 0; row < rows; row++)
		...
		for (col = 0; col < 16; col++)
			...
			if (col < cols)
				...
				dng_local_printf::printf ("%02x ", x [col]);
		...
		for (col = 0; col < 16; col++)
			...
			if (x [col] >= (uint8) ' ' && x [col] <= (uint8) '~')
				{
				dng_local_printf::printf ("%c", x [col]);
				}
	...
	if (count > rows * 16)
		{
		dng_local_printf::printf ("    ... %u more bytes\n", (unsigned) (count - rows * 16));
		}
	...
	while (count > 0)
		...
		if (x == '\n' ||
			...
			if (x >= ' ' && x <= '~')
				...
				dng_local_printf::printf ("%c", (char) x);
				...
				dng_local_printf::printf ("\\%03o", (unsigned) x);
...
void DumpString (const dng_string &s)
	...
	while (*ss != 0 && total++ < kMaxDumpString)
		...
		if (c >= ' ' && c <= '~')
			{
			dng_local_printf::printf ("%c", (char) c);
			}
			...
			case '\r':
				{
				dng_local_printf::printf ("\\r");
				break;
				}
			
			default:
				{
				dng_local_printf::printf ("[%X]", (unsigned) c);
				}
	...
	if (extra > 0)
		{
		dng_local_printf::printf ("...\" (%u more bytes)", (unsigned) extra);
		}
...
void DumpTagValues (dng_stream &stream,
	...
	dng_local_printf::printf ("%s:", tag_name ? tag_name
	...
	switch (tagType)
		...
		case ttDouble:
			...
			if (tagCount > kMaxDumpSingleLine)
				...
				dng_local_printf::printf (" %u entries", (unsigned) tagCount);
			...
			for (uint32 j = 0; j < tagCount && j < kMaxDumpArray; j++)
				...
				if (tagCount <= kMaxDumpSingleLine)
					...
					if (j == 0)
						...
						dng_local_printf::printf (" %s =", entry_name);
				...
				switch (tagType)
					...
					case ttSLong:
						...
						dng_local_printf::printf ("%d", (int) x);
					...
					case ttRational:
						...
						dng_local_printf::printf ("%u/%u", (unsigned) x.n, (unsigned) x.d);
					...
					case ttSRational:
						...
						dng_local_printf::printf ("%d/%d", (int) x.n, (int) x.d);
			...
			if (tagCount > kMaxDumpArray)
				...
				dng_local_printf::printf ("    ... %u more entries\n", (unsigned) (tagCount - kMaxDumpArray));
...
void DumpMatrix (const dng_matrix &m)
	...
	for (uint32 row = 0; row < m.Rows (); row++)
		...
		for (uint32 col = 0; col < m.Cols (); col++)
			...
			dng_local_printf::printf ("%8.4f", m [row] [col]);
...
void DumpVector (const dng_vector &v)
	...
	for (uint32 index = 0; index < v.Count (); index++)
		...
		dng_local_printf::printf (" %0.4f", v [index]);
...
void DumpDateTime (const dng_date_time &dt)
	...
	dng_local_printf::printf ("%04d:%02d:%02d %02d:%02d:%02d",
			(int) dt.fYear,
			(int) dt.fMonth,
			(int) dt.fDay,
			(int) dt.fHour,
...
void DumpExposureTime (real64 x)
	...
	if (x > 0.0)
		...
		if (x >= 0.25)
			{
			dng_local_printf::printf ("%0.2f sec", x);
			}
			
		else if (x >= 0.01)
			{
			dng_local_printf::printf ("1/%0.1f sec", 1.0 / x);
			}
			
		else
			{
			dng_local_printf::printf ("1/%0.0f sec", 1.0 / x);
			}
		...
		dng_local_printf::printf ("<invalid>");
...
void DumpFingerprint (const dng_fingerprint &p)
	...
	for (uint32 j = 0; j < 16; j++)
		{
		dng_local_printf::printf ("%02x", p.data [j]);
		}
			...
			for (uint32 s = skipSat0 ? 1 : 0; s < sats; s++)
				...
				if (gDumpLineLimit == 0 ||
					...
					if (vals == 1)
						...
						dng_local_printf::printf ("    h [%2u] s [%2u]:  h=%8.4f s=%6.4f v=%6.4f\n",
								(unsigned) h,
								(unsigned) s,
								(double) dh,
								(double) ds,
						...
						dng_local_printf::printf ("    v [%2u] h [%2u] s [%2u]:  h=%8.4f s=%6.4f v=%6.4f\n",
								(unsigned) v,
								(unsigned) h,
								(unsigned) s,
								(double) dh,
